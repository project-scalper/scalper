...
async def atr(exchange:ccxt.Exchange, symbol:str, timeframe:str='5m', ohlcv:List=None) -> List:
    """Average True Range of a symbol.
    Args:
    ...
    try:
        ...
        if len(ohlcv):
            ...
            _atr = df.ta.atr(length=length, mamode=mamode, drift=drift,
                             offset=offset)
            df = pd.concat([df, _atr], axis=1)
            df = df.tail(10)
            resp = df.to_dict(orient='records')
            for item in resp:
                item['datetime'] = datetime.fromtimestamp(item['timestamp'] / 1000)
                item['ATR'] = item[f"ATRr_{length}"]
            resp.reverse()
...
async def main():
    ...
    resp = await atr(exchange, symbol, timeframe='5m')
    print(resp)
