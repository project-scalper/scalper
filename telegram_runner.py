...
from helper.user_handler import fetch_eligible_users
...
BOT_USERNAME: Final = "@DennizenBot"
timefmt = "%Y-%m-%d %H:%M:%S"

STATES = {}
# admin_key = handleEnv("admin_key")
subscription_fee = 50
last_sent_sms = datetime.now()
...
#             await Update.message.reply_text("You have already subscribed for this service")
#             return
#         active_trial = await check_user_free_trial_status(user)
...
async def handle_response(orig_text:str, chat_id) -> str:
    ...
    if chat_id in STATES:
        if STATES[chat_id] == "USERNAME":
            ...
            response += "please keep it safe and private. You can use it to recover your account if you lose/change your device\n\n"
            del STATES[chat_id]
            # return response
...
async def send_report(opp:Dict):
    ...
    if "BUY" in opp['signal']:
        result_string = "***New signal***\nLONG 📈📈\n\n"
    elif "SELL" in opp['signal']:
        result_string = "***New signal***\nSHORT 📉📉\n\n"
    else:
        result_string = "**New signal\n\n"
    ...
    for user in users:
        result_string += (
            ...
            f"Take Profit   ➝  `{opp['tp']}`\n"
            f"Stop loss     ➝  `{opp['sl']}`\n"
        )
        messages[user.chat_id] = result_string
...
async def handle_message(update: Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    # response = "Hello there, you are now a Dennizen"
    print("Bot:", response)
    await update.message.reply_text(response)
